...
module RoleRequirementSystem
  ...
  module RoleSecurityClassMethods
    ...
    #    require_role "admin", :only => :update, :unless => "current_admin_user.authorized_for_listing?(params[:id]) "
    #
    # Valid options
    #
    ...
    # This is the core of RoleRequirement.  Here is where it discerns if a admin_user can access a controller or not./
    def admin_user_authorized_for?(admin_user, params = {}, binding = self.binding)
      ...
      self.role_requirements.each{| role_requirement|
        ...
        passed = false
        roles.each { |role|
          passed = true if admin_user.has_role?(role)
        } unless (! admin_user || admin_user==:false)
  ...
  module RoleSecurityInstanceMethods
    ...
    def check_roles       
      return access_denied unless self.class.admin_user_authorized_for?(current_admin_user, params, binding)
      
      true
    end
    
  protected
    # receives a :controller, :action, and :params.  Finds the given controller and runs admin_user_authorized_for? on it.
    ...
    def url_options_authenticate?(params = {})
      ...
      klass.admin_user_authorized_for?(current_admin_user, params, binding)
    end
  end
end