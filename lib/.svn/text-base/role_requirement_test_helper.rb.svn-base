...
module RoleRequirementTestHelper
  ...
  # Makes sure a admin_user can access the given action
  #
  # Example:
  #
  #   assert_admin_user_can_access(:quentin, "index")
  # 
  def assert_admin_user_can_access(admin_user, actions, params = {})
    assert_admin_user_access_check(true, admin_user, actions, params)
  end
  
  # Makes sure a admin_user cant access the given action
  #
  # Example:
  #
  #   assert_admin_user_cant_access(:quentin, "destroy", :listing_id => 1)
  # 
  ...
  # Check a list of admin_users against a set of actions with parameters.
  # 
  # Parameters:
  #   admin_users_access_list - a hash where the key is the label for a fixture, and the value is a boolean.
  ...
  # 
  # Example:
  #   assert_admin_user_access(
  #     {:admin => true, :quentin => false }, 
  #     [:show, :edit], 
  #     {:listing_id => 1})
  def assert_admin_users_access(admin_users_access_list, actions, params = {})
    admin_users_access_list.each_pair {|admin_user, access| 
      assert_admin_user_access_check(access, admin_user, actions, params)
    }
  end
  
  alias :assert_admin_user_cannot_access :assert_admin_user_cant_access

private
  def assert_admin_user_access_check(should_access, admin_user, actions, params = {})
    ...
    (Array===actions ? actions : [actions]).each { |action|
      ...
      login_as admin_user
      if should_access
        assert request_passes_role_security_system?(action, params), "request to #{@controller.class}##{action} with admin_user #{admin_user} and params #{params.inspect} should have passed "
      else
        assert ! request_passes_role_security_system?(action, params), "request to #{@controller.class}##{action} with admin_user #{admin_user} and params #{params.inspect} should have been denied"
      end
    }
  end
